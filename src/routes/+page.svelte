<script lang="ts">
  ...
  onMount(() => {
    ...
    const drawRectWithGradient = (x: number, y: number, w: number, h: number) => {
    ...
    const render = () => {
      ...
      drawText(com.score, canvas.width / 2 + 50, canvas.height - 100, '#222');
      drawNet();
...
<div class="flex flex-col items-center w-screen h-screen bg-slate-700 justify-center">
  <p class="font-medium text-2xl text-gray-100 my-5">Welcome to PONG Battle</p>
  <p class="text-slate-200 h-8">{winner}</p>
  <canvas class="bg-gray-100" width="800" height="600" />
  <div class="flex flex-col items-center justify-evenly">
    <p class="font-medium text-xl text-gray-100">Move your mouse to play, first to 10 wins!</p>
